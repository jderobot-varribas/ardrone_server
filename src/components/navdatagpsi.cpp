...
{
    std::cout << "navgpsdata start" << std::endl;
    memset(&gpsdata_defaults_, 0, sizeof(jderobot::NavdataGPSData));
    gpsdata_defaults_.channels = jderobot::Satellites(jderobot::MaxNumChannels);
    assert(sizeof(jderobot::SatelliteInfo) == sizeof(uint8_t)*2);
}

NavdataGPSI::~NavdataGPSI()
{
    std::cout << "navgpsdata end" << std::endl;
}

jderobot::NavdataGPSData NavdataGPSI::getNavdataGPS(Ice::Current const & c)
{
    vp_os_mutex_lock(&navdata_lock);
            navdata_unpacked_t navdata_raw = *shared_raw_navdata;
    vp_os_mutex_unlock(&navdata_lock);

    jderobot::NavdataGPSData gpsdata = gpsdata_defaults_;
    ...
    memcpy(gpsdata.channels.data(), &(navdata_raw.navdata_gps_info.channels[0]), 12*sizeof(uint8_t)*2); //TODO: keep an eye on it!
